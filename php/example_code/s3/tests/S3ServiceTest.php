...
class S3ServiceTest extends TestCase
{
    ...
    public function testListObjects()
    {
        $exception = new AwsException("test message", $this->createMock(CommandInterface::class));
        $testResult = $this->createMock(Result::class);
        $this->client->expects($this->exactly(2))->method('__call')->with('listObjects')->will(
            $this->onConsecutiveCalls($testResult, $this->throwException($exception))
        );
    ...
    {
        $this->client->expects($this->exactly(3))->method('__call')->with('listObjects')->will(
    ...
    {
        $exception = new AwsException("test message", $this->createMock(CommandInterface::class));
        $this->client->expects($this->exactly(4))->method('__call')->will(
            $this->onConsecutiveCalls($this->endResult, true, true, $this->throwException($exception))
        );
